				...
				<div class="row">
					<div class="row draggable-portlets">
						...
						<table class="table table-bordered datatable" id="table-1">
							...
							<tbody>
								<%
									ArrayList<AnswerVo> ls = (ArrayList) session.getAttribute("list");
									System.out.println(ls.size());
									for (AnswerVo av : ls) {

										SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
										String strDate = av.getDate();
										try {
								...
								<tr class="odd gradeX">
									<td><%=av.getAnsID()%></td>
									<td><%=av.getQueID()%></td>
									<td><%=av.getDepartment()%></td>
									<td><%=av.getSemester()%></td>
									<td><%=av.getName()%></td>
									<td><%=strDate%></td>
									<td><%=av.getAnswer()%></td>
									<td><a
										href="<%=request.getContextPath()%>/CTServlet?type=answer&operation=delete&id=<%=av.getAnsID()%>"
										...
										<button class="btn btn-info btn-sm btn-icon icon-left"
											...
											data-placement="left"
											data-original-title="<%=av.getAnswer()%>">
