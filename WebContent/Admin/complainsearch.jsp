...
<title>Campus Talk | complain</title>
				...
				<div class="row">
					<div class="row draggable-portlets">
						...
						<table class="table table-bordered datatable" id="table-1">
							...
							<tbody>
								<%
									ArrayList<ComplainVo> ls = (ArrayList) session.getAttribute("list1");

									for (ComplainVo cv : ls) {
										SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
										Date date = sdf.parse(cv.getComplain_date());
										sdf = new SimpleDateFormat("dd MMM yyyy");
										String strDate = sdf.format(date);
								%>
								<tr class="odd gradeX">
									<td><%=cv.getComplain_id()%></td>
									<td><%=cv.getComplain_enrollment()%></td>
									<td><%=cv.getComplain_name()%></td>
									<td><%=cv.getComplain_department()%></td>
									<td><%=cv.getComplain_semester()%></td>
									<td><%=strDate%></td>
									<td><a
										href="<%=request.getContextPath()%>/CTServlet?type=complain&operation=delete&id=<%=cv.getComplain_id()%>"
										...
										<button class="btn btn-info btn-sm btn-icon icon-left"
											...
											data-placement="left"
											data-content="<%=cv.getComplain_description()%>"
											data-original-title="<%=cv.getComplain_title()%>">
